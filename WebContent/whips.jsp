...
<table>
	<thead>
		...
		<c:forEach var="whip" items="${whipList}">
			
			<tr>
				<td>${whip.name}</td>
				<td>${whip.attackRating}</td>
				<td>${whip.damageType}</td>
				<td>${whip.weaponClass}</td>
				<td>
					<form class="removeButton" action="removeWhip.do" method="POST">
						<input type="hidden" value="${whip.name}" name="weapon"/>
